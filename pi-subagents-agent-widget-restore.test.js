...
test("restoreKnownPatch removes ai-harnesses click patch from installed agent widget", () => {
  ...
  const restored = restoreKnownPatch(fs.readFileSync(installed, "utf8"));
  assert.doesNotMatch(restored, /getMarkdownTheme/);
  assert.doesNotMatch(restored, /click to view/);
  assert.doesNotMatch(restored, /piMouseCapture/);
  assert.doesNotMatch(restored, /openFooterAgentSession/);
  assert.match(restored, /setWidget\("agents"/);
  assert.match(restored, /setStatus\("subagents"/);
});
